  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-4 sm:px-6 sm:py-5">
      ...
      <section className="mt-4">
        <h2 className="mb-1.5 text-[10px] font-semibold uppercase tracking-wide text-muted">
          Upcoming · {upcoming.length}
        </h2>
        <ul className="divide-y divide-card-border overflow-hidden rounded-md border border-card-border bg-card">
          {upcoming.map((ev) => (
            <li key={ev.id} className="px-3 py-2 hover:bg-card-hover sm:px-3.5">
              <div className="flex flex-wrap items-baseline justify-between gap-2">
                <h3 className="text-[13px] font-semibold tracking-tight">
                  {ev.title}
                </h3>
                <span className="rounded-sm border border-card-border px-1 py-px text-[10px] capitalize text-muted">
                  {ev.type}
                </span>
              </div>
              <p className="mt-0.5 text-[11px] text-accent">{ev.when}</p>
      ...
      <section className="mt-5">
        ...
        <ul className="divide-y divide-card-border overflow-hidden rounded-md border border-card-border opacity-90">
          {past.map((ev) => (
            <li key={ev.id} className="px-3 py-2 sm:px-3.5">
              <div className="flex flex-wrap items-baseline justify-between gap-2">
                <h3 className="text-[13px] font-semibold tracking-tight">
                  {ev.title}
                </h3>
